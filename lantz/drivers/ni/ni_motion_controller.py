    ...
    def move(self, point):
        ...
        if step_voltages.size:
            self.ao_motion_task.timing.cfg_samp_clk_timing(
                self.acq_rate.to('Hz').m, #self.acq_rate.to('Hz').m,
            ...
            # TODO timeout
            sample_writer_stream.write_many_sample(ni_sample_buffer, timeout=60)

            self.ao_motion_task.start()
            time.sleep((1.1*step_voltages.shape[0] / self.acq_rate).to('s').m) #.to('s').m
